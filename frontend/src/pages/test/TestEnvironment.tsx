...
const TestEnvironment: React.FC = () => {
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-background text-foreground">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground">Failed to load test or no questions found.</p>
  ...
  if (participant?.status === 'submitted' || isTerminated) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-background text-foreground p-4">
        <Card className="w-full max-w-md bg-card border-border">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl text-black">Test Submitted</CardTitle>
            <CardDescription>
              {isTerminated 
          ...
          <CardContent className="space-y-4">
            <div className="bg-white p-4 rounded-lg text-sm text-black text-center">
  ...
  if (!isProctoringAgreed) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-background text-foreground p-4 z-50">
         <AlertDialog open={true}>
          <AlertDialogContent className="bg-card border-border text-foreground">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-muted-foreground space-y-3 pt-4">
                <p className="text-center text-lg font-semibold text-black">The test environment is being monitored</p>
                <div className="bg-red-500 border border-red-500/20 rounded-lg p-4 space-y-3 text-sm text-white">
                ...
                <div className="bg-blue-500 border border-blue-500/20 rounded p-3 text-sm text-blue-200 mt-4">
                  <p className="font-medium text-white">After final submission:</p>
    ...
    <div className="fixed inset-0 flex flex-col bg-background text-foreground overflow-hidden">
      ...
      <header className="flex h-14 items-center justify-between border-b border-white/5 bg-background px-4 shrink-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-semibold text-black">{test.name}</h1>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="font-bold text-black">{formatTime(timeLeft)}</span>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-hidden relative">
        <ResizablePanelGroup direction="horizontal">
          ...
          <ResizablePanel defaultSize={30} minSize={20} maxSize={50}>
            <div className="h-full flex flex-col bg-white border-r border-white/5">
              {/* Question Navigation */}
              <div className="flex items-center justify-between border-b border-white/5 p-3 shrink-0">
                ...
                <div className="flex gap-1">
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-black hover:text-white hover:bg-white/5"
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="prose prose-invert prose-sm max-w-none">
                      <ReactMarkdown 
                        components={{
                           p: ({node, ...props}) => <p className="text-black leading-relaxed whitespace-pre-wrap" {...props} />,
                           code: ({node, className, children, ...props}) => (
                             <code className={`${className} bg-zinc-800 px-1 py-0.5 rounded text-zinc-200`} {...props}>{children}</code>
                           ),
                           pre: ({node, children, ...props}) => (
                              <pre className="bg-card/50 border border-white/5 p-3 rounded-lg overflow-x-auto" {...props}>{children}</pre>
                           )
                        }}
                      >
                  ...
                  <div className="space-y-4">
                    ...
                    {question.testcases
                      ...
                      .map((tc, idx) => (
                        <div
                          key={tc.id}
                          className="rounded-lg border border-white/5 bg-card/50 p-4"
                        >
                          <p className="text-xs font-medium text-black mb-2 uppercase tracking-wide">Example {idx + 1}</p>
                          <div className="space-y-2 font-mono text-sm">
                            <div>
                              <span className="text-black select-none">Input: </span>
                              <span className="text-black">{tc.input}</span>
                            </div>
                            <div>
                              <span className="text-black select-none">Output: </span>
                              <span className="text-black">{tc.expected_output}</span>
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            <ResizablePanelGroup direction="vertical">
              ...
              <ResizablePanel defaultSize={70} minSize={30}>
                 <div className="h-full flex flex-col bg-white">
                   <div className="flex items-center justify-between border-b border-white/5 bg-white px-4 py-2 shrink-0">
