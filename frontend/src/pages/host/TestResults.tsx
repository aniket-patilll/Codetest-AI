    ...
    <div className="min-h-screen bg-background text-foreground p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-border">
        <div className="space-y-1">
          <Link to="/host" className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-primary mb-2 transition-colors">
            <ArrowLeft className="h-4 w-4" />
            Back to Dashboard
          </Link>
          <div className="flex items-center gap-3">
             <h1 className="text-3xl font-bold tracking-tight text-foreground glow-text">Test Results</h1>
             <Badge variant="outline" className="border-border bg-card/50 text-muted-foreground font-mono">
                {test?.name}
             </Badge>
          </div>
        ...
        <div className="flex gap-3">
          ...
          <Button 
             ...
             className={cn(
                "font-semibold shadow-lg shadow-primary-900/20 transition-all",
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-end">
          <div className="w-full md:max-w-md relative">
             ...
             <Input 
                ...
                className="pl-10 bg-card border-border text-foreground placeholder:text-zinc-600 focus-visible:ring-emerald-500/50"
                value={searchQuery}
          ...
          <div className="flex gap-2 w-full md:w-auto">
             ...
             <Select value={sortDirection} onValueChange={(v: any) => setSortDirection(v)}>
                <SelectTrigger className="w-[110px] bg-card border-border text-foreground">
           ...
           <Table>
              <TableHeader className="bg-card/80 border-b border-border">
                 <TableRow className="hover:bg-transparent border-border">
                    <TableHead className="text-muted-foreground font-medium pl-6">Student</TableHead>
                    <TableHead className="text-muted-foreground font-medium">Language</TableHead>
                    <TableHead className="text-muted-foreground font-medium text-right">Score</TableHead>
                    <TableHead className="text-muted-foreground font-medium text-right">Test Cases</TableHead>
                    <TableHead className="text-muted-foreground font-medium text-right hidden lg:table-cell">Complexity</TableHead>
                    <TableHead className="text-muted-foreground font-medium text-right pr-6">Details</TableHead>
                 </TableRow>
              </TableHeader>
              <TableBody>
                       ...
                       return (
                          <TableRow key={sub.id} className="border-border hover:bg-muted/30 transition-colors">
                             <TableCell className="pl-6">
                                <div>
                                   <div className="font-medium text-foreground">{getStudentName(sub)}</div>
                                   <div className="text-xs text-zinc-500">{getStudentEmail(sub)}</div>
                                </div>
                             </TableCell>
                             <TableCell>
                                <Badge variant="outline" className="bg-card text-muted-foreground border-zinc-700 capitalize font-mono text-[10px]">
                                   {sub.language}
                                </Badge>
                             </TableCell>
      ...
      <Sheet open={!!selectedSubmission} onOpenChange={() => setSelectedSubmission(null)}>
        <SheetContent className="w-full sm:max-w-2xl bg-card border-border text-foreground overflow-y-auto p-0">
          {selectedSubmission && (() => {
                  ...
                  <div className="p-6 space-y-8 pb-20">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl border border-border bg-card/50">
                           <div className="text-black text-xs font-medium uppercase tracking-wider mb-2">Final Score</div>
                        ...
                        <div className="p-4 rounded-xl border border-border bg-card/50">
                           <div className="text-black text-xs font-medium uppercase tracking-wider mb-2">Test Cases</div>
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-5 space-y-6">
                              ...
                              <div className="grid grid-cols-2 gap-4 pb-4 border-b border-indigo-500/10">
                                 <div>
                                    <div className="text-xs text-black mb-1">Time Complexity</div>
                                    <div className="font-mono text-black font-semibold">{aiEval.time_complexity}</div>
                                 </div>
                                 <div>
                                    <div className="text-xs text-black mb-1">Space Complexity</div>
                                    <div className="font-mono text-black font-semibold">{aiEval.space_complexity}</div>
                                 </div>
                              </div>
                              ...
                              <div className="space-y-3">
                                 ...
                                 <div>
                                    <div className="flex justify-between text-sm mb-1.5">
                                       <span className="text-black">Logical Clarity</span>
                              ...
                              {aiEval.justification && (
                                 <div className="pt-4 border-t border-indigo-500/10">
                                    <div className="flex items-start gap-2 mb-2">
                                       ...
                                       <div className="text-xs text-indigo-500 font-semibold uppercase tracking-wider">Evaluation Summary</div>
                                    </div>
                                    <p className="text-sm text-black leading-relaxed pl-6">
                                       {aiEval.justification}
                                    </p>
                                 </div>
                              )}
                              ...
                              {aiEval.suggestions && aiEval.suggestions.length > 0 && (
                                 <div className="pt-4 border-t border-indigo-500/10">
                                    <div className="flex items-center gap-2 mb-3">
                                       ...
                                       <div className="text-xs text-amber-500 font-semibold uppercase tracking-wider">Improvement Suggestions</div>
                                    </div>
                                    <ul className="space-y-2 pl-6">
                                       {aiEval.suggestions.map((suggestion, idx) => (
                                          <li key={idx} className="text-sm text-black leading-relaxed flex items-start gap-2">
                     ...
                     <div className="space-y-3">
                        <div className="flex items-center justify-between">
                           <h3 className="font-bold text-lg text-foreground">Submitted Code</h3>
                        ...
                        <div className="rounded-xl border border-border bg-muted overflow-hidden">
                           <div className="bg-muted/50 px-4 py-2 border-b border-border flex justify-between items-center">
                              ...
                              <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-500 hover:text-foreground">
