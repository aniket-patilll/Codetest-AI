...
const TestOutput: React.FC<TestOutputProps> = ({ 
  ...
  return (
    <div className="h-full flex flex-col bg-white relative border-t border-border">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-white/5 shrink-0">
      ...
      <div className="flex-1 overflow-hidden font-mono text-sm relative">
        {isRunning || isSubmitting || isRunningAllTests ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm z-10 transition-all text-zinc-400 gap-3">
        ...
        {!result ? (
           <div className="h-full flex flex-col items-center justify-center text-black p-8 text-center">
           ...
           <div className="h-full flex flex-col">
             ...
             <div className="px-4 py-2 bg-white border-b border-white/5 text-xs text-black">
               Showing {result.testcases.length} test cases
             </div>
             
             {/* Test Case Tabs */}
             <div className="flex items-center gap-2 p-2 px-4 overflow-x-auto border-b border-white/5 bg-white">
                 ...
                 <button
                   ...
                   className={cn(
                     ...
                     activeCase === idx 
                       ...
                       : "text-black hover:text-primary hover:bg-primary/50"
                   )}
                 >
                   {tc.status === 'passed' ? (
             ...
             <div className="flex-1 overflow-auto p-4 space-y-4">
               {/* Complexity Metrics Display - Moved Inside */}
               {result.complexity && (
                 <div className="p-3 bg-white border border-black/10 rounded-md mb-2">
                   <div className="grid grid-cols-2 gap-3 text-xs">
                     <div className="flex items-center gap-2">
                       <Clock className="h-4 w-4 text-blue-400" />
                       <div>
                         <div className="text-black">Time Complexity</div>
                         <div className="font-mono text-black">{result.complexity.time}</div>
                       </div>
                     </div>
                     <div className="flex items-center gap-2">
                       <Zap className="h-4 w-4 text-purple-400" />
                       <div>
                         <div className="text-black">Space Complexity</div>
                         <div className="font-mono text-black">{result.complexity.space}</div>
                       </div>
                     </div>
                     <div className="flex items-center gap-2">
                       <Clock className="h-4 w-4 text-green-400" />
                       <div>
                         <div className="text-black">Execution Time</div>
                         <div className="font-mono text-black">{result.complexity.executionTime}</div>
                       </div>
                     </div>
                     <div className="flex items-center gap-2">
                       <Zap className="h-4 w-4 text-orange-400" />
                       <div>
                         <div className="text-black">Memory Used</div>
                         <div className="font-mono text-black">{result.complexity.memoryUsed}</div>
                       </div>
                     </div>
                   </div>
                   ...
                   {result.ai_evaluation && (
                       <div className="mt-3 pt-3 border-t border-black/10">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs mb-2">
                              <div>
                                  <span className="text-zinc-500">Quality Score:</span> <span className="text-black">{result.ai_evaluation.code_quality_score}/10</span>
                              </div>
                              <div>
                                  <span className="text-zinc-500">Logical Clarity:</span> <span className="text-black">{result.ai_evaluation.logical_clarity_score}/10</span>
                              </div>
                          </div>
                       </div>
                   )}
                 ...
                 <div className="space-y-4 animate-in fade-in duration-300">
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <span className="text-xs text-black uppercase tracking-wide">Input</span>
                        <div className="p-3 rounded bg-white border border-black text-black">
                          {result.testcases[activeCase].input}
                        </div>
                      </div>

                      <div className="space-y-1">
                        <span className="text-xs text-black uppercase tracking-wide">Output</span>
                         <div className={cn(
                           "p-3 rounded border text-black",
                           result.testcases[activeCase].status === 'passed' 
                      ...
                      <div className="space-y-1">
                        <span className="text-xs text-black uppercase tracking-wide">Expected</span>
                        <div className="p-3 rounded bg-white border border-black text-black">
      ...
      {/* Footer Actions */}
      <div className="p-2 border-t border-white/5 bg-white flex items-center justify-between shrink-0">
        <Button
          onClick={onClear}
          variant="ghost"
          size="sm"
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="bg-primary hover:bg-primary/80 text-white h-8 px-4 text-xs font-semibold shadow-lg shadow-green-900/20"
          >
            {isFinalSubmitting ? (
              <>
                <Loader2 size={14} className="mr-2 animate-spin" />
